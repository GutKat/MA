...
class PFFunctionAdapter(libinfrared.Function):
    ...
    def __init__(self, wf):
        ...
        super().__init__(wf.vars(), wf.weight, wf.cache_in_board)
        self._wf = wf
...
class WeightedFunction:
    ...
    def __init__(self, variables, cache_in_board=False):
        self._vars = variables
        self._weight = 0
        self._cache_in_board = cache_in_board
    ...
    @property
    def cache_in_board(self):
        return self._cache_in_board

    @weight.setter
    def weight(self, weight):
        self._weight = weight

    @cache_in_board.setter
    def cache_in_board(self, cache_in_board):
        self._cache_in_board = cache_in_board
...
class Model:
    ...
    def add_functions(self, functions, group='base', cache_in_board=False):
        ...
        for function in functions:
            function._cache_in_board = cache_in_board

        self._functions[group].extend(copy.deepcopy(functions))

    def delete_functions(self, group):
        """Removes all functions attached to the given group
        Args:
            group: indentifier of function group. 
        """

        if group not in self._functions:
            return

        del self._features[group]
        del self._functions[group]
    ...
    def add_feature(self, name, group, eval_fun, cache_in_board=False):
        ...
        """
        self._features[name] = Feature(name, eval_fun, group=group, cache_in_board=cache_in_board)
        self.set_cache_in_board_of_feature(cache_in_board, name)
    ...
    def set_cache_in_board_of_feature(self, cache_in_board, name):
        """Set the weight of a feature and its function group.
        ...
        Args:
            ...
            name: the feature name
        """
        self.features[name]._cache_in_board = cache_in_board
        groups = self.features[name].group
        if type(groups) != list:
            groups = [groups]
        for group in groups:
            self.features[group]._cache_in_board = cache_in_board
            for f in self._functions[group]:
                f._cache_in_board = cache_in_board
    ...
    def get_feature_weight(self, group):
        """Get the weight of a feature

        Args:
            name: the feature name
        """
        return self.features[group]._weight
            ...
            if bagidx not in children:  # --> bagidx is a root
                ...
                while stack:
                    ...
                    for x in bagfunctions[i]:
                        x.cpp_pointer = self._ct.add_function(cluster, self._EA.function(x))
                        x.cluster = cluster

                    for j in td.adj[i]:
                        children.add(j)
                        stack.append((cluster, j))
        # print("Fill Tree done", flush=True)
...
class Feature:
    ...
    def __init__(self, identifier, eval_function, group=[], cache_in_board=False):
        """Construct feature
...
class EngineBase(ABC):
    ...
    def update_messages(self):
        ...
        if (not self.recompute_messages):
            return None
        # print("update_message2")
        self.recompute_messages = False
        # self._ct.clear_messages()
        return self._ct.reevaluate()
    ...
    def change_feature_weight(self, new_weight, name):
        ...
        self.recompute_messages = True

    def replace_feature_functions(self, name, new_functions, feature_weight):
        """Replaces all functions of a given feature with new functions. Modifies
        both model stored in the sampler and the cluster tree of the sampler.
        Does not update the initial model. A call to this function can result in
        the initial model being out of sync.

        This function should only be used when the new functions are a 1:1 substitution for the old functions.
        The new functions must be provided in the exact same order as the functions that they are replacing.
        The feature must not contains any other functions other than the ones to be replaced.
        This will produce unexpected results in any other case.
        An example would be to use the same distance function but with a different sequence to compare to.

        Args:
            name: Name of the feature where the function sould be replaces
            new_functions: A list containing the new functions
        """

        old_functions = self._model._functions[name]
        assert (len(new_functions) == len(old_functions))

        if (self._ct is not None):
            # print(self._ct._ct.has_message())
            for new_f, old_f in zip(new_functions, old_functions):
                assert (new_f._vars == old_f._vars)
                new_f._weight = feature_weight
                new_f.cache_in_board = old_f.cache_in_board
                new_f.cluster = old_f.cluster
                new_f.cpp_pointer = self._ct._ct.replace_function(self._ct._EA.function(new_f), old_f.cpp_pointer, old_f.cluster)
            self.recompute_messages = True

        self._model.delete_functions(name)
        self._model.add_functions(new_functions, name)
        self._model.set_feature_weight(feature_weight, name)
...
class BoltzmannSampler(EngineBase):
    ...
    def resample(self, variables, assignment):
        ...
        Note:
            raises exception ConsistencyError if the model is inconsistent.
        """
        # print("python_resample")
        self.setup_engine()
        self.reevaulate()
        self.update_messages()
        # print("python_resample2")
        return self._ct.resample(variables, assignment)
...
def updateNeighborSampler(neighborSampler, newValues, distanceFunction, shift, variance):
    """Updates the given Neighbor sampler by replacing the current distance function with the
    distance function given here. Then lets the new distance function target the given
    shift and variance.

    The distance function will be applied to each variable independently and and must take a second argument as
    input. This second argument is supplied through the list "initialValues". This means when the funtion is applied
    to variable 6, it will be given initialValues[6] as a second argument.

    The given neighborSampler must contains a feature calles 'dist' with a distance function that
    is equivalent to newly given distance function. Such a neighbor sampler can be obtained from an initial
    sampler by calling "getNeighborSampler" with the same distance function.
    If a different distance function is used, both functions need to be 1:1 substitutable.

    Args:
        neighborSampler: The sampler containing the distance function that will be replaced.
        newValues: A list with length equal to the number of variables. The distance function for each variable will be supplied the corresponding element of this list.
        distanceFunction: A distance funtion which will be applied to each variable. Must take one additional argument.
        shift: The shift to target with the distance funtion. Given as a fraction of the number of variables.
        variance: The variance allowed for the distance funtion. Given as a fraction of the number of variables.
    """
    seqlen = len(newValues)
    currentWeight = neighborSampler._model.get_feature_weight('dist')
    neighborSampler.replace_feature_functions('dist', [distanceFunction(i, newValues[i]) for i in range(seqlen)], currentWeight)
    # neighborSampler.replace_feature_functions('dist', [distanceFunction(i, 1) for i in range(seqlen)], currentWeight)
    neighborSampler.set_target(shift * seqlen, variance * seqlen, 'dist')


def getNeighborSampler(initialSampler, initialValues, distanceFunction, shift, variance):
    """Creates a neighbor sampler from a given sampler. For the neighbor sampler, the given function
    "distanceFunction" is added as an additional function.

    The distance function will be applied to each variable independently and and must take a second argument as
    input. This second argument is supplied through the list "initialValues". This means when the funtion is applied
    to variable 6, it will be given initialValues[6] as a second argument.

    Those function will be grouped into the feature 'dist'. This name should therefore not be used otherwise.


    Args:
        initial: The inital sampler from which the neighbor sampler should be created
        inital: A list with length equal to the number of variables. The distance function for each variable will be supplied the corresponding element of this list.
        distanceFunction: A distance funtion which will be applied to each variable. Must take one additional argument.
        shift: The shift to target with the distance funtion. Given as a fraction of the number of variables.
        variance: The variance allowed for the distance funtion. Given as a fraction of the number of variables.

    Returns:
        A new neighbor sampler
    """
    seqlen = len(initialValues)

    neighborSampler = copySampler(initialSampler)

    # print("add dist", flush=True)
    # print(initialValues)
    neighborSampler.model.add_functions([distanceFunction(i, initialValues[i]) for i in range(seqlen)], group='dist', cache_in_board=True)
    # neighborSampler.model.add_functions([distanceFunction(i, 1) for i in range(seqlen)], group='dist', cache_in_board=True)
    neighborSampler.change_feature_weight(1, 'dist')
    neighborSampler.set_target(shift * seqlen, variance * seqlen, 'dist')

    return neighborSampler


def copySampler(initialSampler):
    """Deepcopies a sampler including the cluster tree

    Args:
        initialSampler: The initial sampler that will be copied.
    Returns:
        A copy of the Sampler
    """

    neighborSampler = Sampler(initialSampler.model)

    neighborSampler.callback = initialSampler.callback
    neighborSampler.cooling = initialSampler.cooling
    neighborSampler.samples_per_round = initialSampler.samples_per_round
    neighborSampler.tweak_factor = initialSampler.tweak_factor
    neighborSampler.verbose = initialSampler.verbose
    neighborSampler._td = copy.deepcopy(initialSampler._td)
    neighborSampler._td_factory = initialSampler._td_factory

    # neighborSampler._ct = (initialSampler._ct)
    # neighborSampler._ct._bagsets = copy.deepcopy(initialSampler._ct._bagsets)
    # print(initialSampler._ct._ct.has_message())
...
def neighbor_optimize(sampler, objective, steps, temp, shift, variance, dist_func, start=None):
    ...
    # print("1")
    currSampler = sampler
    currSample = currSampler.sample() if start is None else start
    ...
    for _ in range(steps):
        ...
        if (currSample == newSample):
            # print("7")
            updateNeighborSampler(currSampler, currSample.values(), dist_func, shift, variance)
            # print("8")
