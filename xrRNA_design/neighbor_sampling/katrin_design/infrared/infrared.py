...
class EngineBase(ABC):
    ...
    def replace_feature_functions(self, name, new_functions, need_replacement, feature_weight):
        ...
        if (self._ct is not None):
            for new_f, old_f, replace in zip(new_functions, old_functions, need_replacement):
                ...
                if replace is True:
                    ...
                    new_f.cpp_pointer = old_f.cpp_pointer
            self.recompute_messages = True
...
class MultiDimensionalBoltzmannSampler(BoltzmannSampler):
    ...
    def set_target(self, target, tolerance, featureid, initial_weight=1):
        ...
        self._model.set_feature_weight(initial_weight, featureid)
...
def updateNeighborSampler(neighborSampler, newValues, oldValues, distanceFunction, shift, variance):
    ...
    shift and variance. Only replaces a function where the new and old values differ. If you want
    all functions to be replaced, pass a vector containing non-valid vales for oldValues.
    ...
    neighborSampler.replace_feature_functions('dist', [distanceFunction(i, newValues[i]) for i in range(seqlen)], [newValues[i] != oldValues[i] for i in range(seqlen)], currentWeight)
